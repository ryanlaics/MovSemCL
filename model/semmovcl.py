...
from model.hier_sem_encoding import HSE

class SemMovCL(nn.Module):

    def __init__(self):
        super(SemMovCL, self).__init__()

        encoder_q = HSE(Config.seq_embedding_dim, 
        ...
        encoder_k = HSE(Config.seq_embedding_dim, 
                                            Config.trans_hidden_dim, 
                                            Config.trans_attention_head, 
                                            Config.trans_attention_layer, 
                                            Config.trans_attention_dropout, 
    ...
    def load_checkpoint(self):
        checkpoint_file = '{}/{}_SemMovCL_best{}.pt'.format(Config.checkpoint_dir, Config.dataset_prefix, Config.dumpfile_uniqueid)
...
class SemMovCLTrainer:

    def __init__(self, str_aug1, str_aug2):
        super(SemMovCLTrainer, self).__init__()
        ...
        self.train_dataloader = DataLoader(train_dataset, 
                                            batch_size = Config.semmovcl_batch_size,
                                            shuffle = False, 
                                            num_workers = 0, 
                                            drop_last = True, 
                                            collate_fn = partial(collate_and_augment, cellspace = self.cellspace, embs = self.embs, augfn1 = self.aug1, augfn2 = self.aug2) )
        
        self.model = SemMovCL().to(Config.device)
        self.checkpoint_file = '{}/{}_SemMovCL_best{}.pt'.format(Config.checkpoint_dir, Config.dataset_prefix, Config.dumpfile_uniqueid)
    ...
    def train(self):
        ...
        optimizer = torch.optim.Adam(self.model.parameters(), lr = Config.semmovcl_training_lr, weight_decay = 0.0001)
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size = Config.semmovcl_training_lr_degrade_step, gamma = Config.semmovcl_training_lr_degrade_gamma)
        
        best_loss_train = 100000
        best_epoch = 0
        bad_counter = 0
        bad_patience = Config.semmovcl_training_bad_patience
        
        for i_ep in range(Config.semmovcl_training_epochs):
            _time_ep = time.time()
            loss_ep = []
            train_gpu = []
            train_ram = []
            ...
            if bad_counter == bad_patience or (i_ep + 1) == Config.semmovcl_training_epochs:
    ...
    def test(self):
        ...
        with open(Config.dataset_file + '_newsimi_raw.pkl', 'rb') as fh:
            ...
            results = []
            for n_db in range(20000,100101,20000):
                rank = torch.sum(torch.le(dists[:,0:n_db].T, targets)).item() / len(q_lst)
                results.append(rank)
            # Assign the results to r1, r2, r3, r4, r5
            r1, r2, r3, r4, r5 = results
            logging.info('[EXPFlag]task=newsimi,encoder=SemMovCL,varying=dbsize,r1={:.3f},r2={:.3f},r3={:.3f},r4={:.3f},r5={:.3f}' \
        ...
        for vt in ['downsampling', 'distort']:
            ...
            r1, r2, r3, r4, r5 = results
            logging.info('[EXPFlag]task=newsimi,encoder=SemMovCL,varying={},r1={:.3f},r2={:.3f},r3={:.3f},r4={:.3f},r5={:.3f}' \
                                          .format(vt, r1, r2, r3, r4, r5))
        return
